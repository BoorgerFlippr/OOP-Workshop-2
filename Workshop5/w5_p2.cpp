...
int main(int argc, char** argv)
{
	...
	auto updatePrice = [usdToCadRate, gbpToCadRate](Book& bk)
	{
		if (bk.country() == "US")
			bk.price() = bk.price() * usdToCadRate;
		else if (bk.country() == "UK")
		{
			if (bk.year() <= 1999 && bk.year() >= 1990)
	...
	if (argc > 2) {
		...
		if (mvFile)
		{
			do
			{
				std::getline(mvFile, mvLine);
				if (mvFile)
				{
					if (mvLine[0] != '#')
					{
						//std::cout << mvLine << std::endl;
						movies[mvCnt] = sdds::Movie(mvLine);
						//std::cout << movies[mvCnt] << std::endl;
						++mvCnt;
					}
				}
			} while (mvFile);
